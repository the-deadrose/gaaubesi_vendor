...
class OrderPermissionSectionWidget extends StatefulWidget {
  final String title;
  ...
  const OrderPermissionSectionWidget({
    super.key,
    required this.title,
  ...
  @override
  State<OrderPermissionSectionWidget> createState() =>
      _OrderPermissionSectionWidgetState();
}

class _OrderPermissionSectionWidgetState
    extends State<OrderPermissionSectionWidget> {
  @override
  void initState() {
    super.initState();
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            BlocBuilder<StaffOrderPermissionBloc, StaffOrderPermissionState>(
              builder: (context, state) {
                if (state is StaffOrderPermissionLoading) {
                  return const Center(child: CircularProgressIndicator());
                } else if (state is StaffOrderPermissionError) {
                  return Center(
                    child: Text(
                      'Error: ${state.message}',
                ...
                } else if (state is StaffOrderPermissionEmpty) {
                  return const Center(
                      child: Text('No permissions available'));
                } else if (state is StaffOrderPermissionLoaded) {
                    ...
                    children: [
                      ...
                      if (widget.selectedPermissions.isNotEmpty)
                        SizedBox(
                          ...
                          child: BlocBuilder<StaffOrderPermissionBloc,
                              StaffOrderPermissionState>(
                            builder: (context, state) {
                              final isUpdating =
                                  state is StaffOrderPermissionUpdating;
